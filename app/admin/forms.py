...
from app.models import Admin, Auth, Role, Tag  # 导入数据模型

auth_list = Auth.query.all()
role_list = Role.query.all()
tags = Tag.query.all()  # 查询标签
...
class AdminForm(FlaskForm):
    ...
    repwd = PasswordField(
        label="重复管理员密码",
        validators=[
            DataRequired("请重复管理员重复密码！"),
            EqualTo('pwd', message="两次密码不一致")
        ],
        description="重复密码",
        render_kw={
            "class": "form-control",
            "placeholder": "请重复管理员重复密码！",
        }
    )
    role_id = SelectField(
        label="所属角色",
        coerce=int,
        choices=[(v.id, v.name) for v in role_list],  # 生成角色列表，列表有角色id和名字比如标签管理员什么的 超级管理员
        render_kw={
            "class": "form-control",
        }
    )
